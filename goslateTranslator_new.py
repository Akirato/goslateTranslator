...
if __name__ == "__main__":
    if (len(sys.argv)) == 1:
	    ...
	    print("Give the sentence and LangId")
        else:
            print("Give the language id for translation.")
    elif len(sys.argv) == 3:
        if sys.argv[1] == "getIdOfForLang":
            print(languageOfId(sys.argv[2]))
	if sys.argv[1] == "translate":
	    print("Give language id for translation")
        else:
            translateSRT(sys.argv[1],None,sys.argv[2])
    elif len(sys.argv) > 3:
    	if sys.argv[1] == "translate":
	    print(translate(sys.argv[2],None,sys.argv[3]).encode('utf-8'))
	else:
